 ...
 * 
 * A special reader decorater which supports more sophisticated access to the
 * underlying reader object.
 * 
 * In particular the reader supports a look-ahead option, which allows you to
 * see the next char returned by next(). Furthermore the skip-method supports
 * skipping until (but excluding) a given char. Similar functionality is
 * supported by the reader as well.
 * 
 */
public class ExtendedBufferedReader extends BufferedReader {
	...
	public ExtendedBufferedReader(Reader r) {
		super(r);
		/*
		 * note uh: do not fetch the first char here, because this might block
		 * the method!
		 */
	}
	...
	/**
	 * Non-blocking reading of len chars into buffer buf starting at
	 * bufferposition off.
	 * 
	 * performs an iteratative read on the underlying stream as long as the
	 * following conditions hold: - less than len chars have been read - end of
	 * stream has not been reached - next read is not blocking
	 * 
	...
	public long skip(long n) throws IllegalArgumentException, IOException {
		...
		lookaheadChar = super.read();
		// fixme uh: we should check the skiped sequence for
		// line-terminations...
	 ...
	 * 
	 * Remember the next char read by read(..) will always be identical to
	 * lookAhead().
	 * 
	...
	/**
	 * Returns the nof line read ATTENTION: the skip-method does invalidate the
	 * line-number counter
	 * 
	...
	public boolean markSupported() {
		/*
		 * note uh: marking is not supported, cause we cannot see into the
		 * future...
		 */
		return false;
	}
