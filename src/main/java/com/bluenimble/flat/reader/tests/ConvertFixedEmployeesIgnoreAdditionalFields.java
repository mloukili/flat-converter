package com.beesphere.flat.reader.tests;

import java.io.File;
import java.io.FileInputStream;
import java.io.FileOutputStream;
import java.io.IOException;
import java.io.InputStreamReader;

import com.beesphere.flat.reader.FlatReaderException;
import com.beesphere.flat.reader.impls.xml.FixedLengthToXmlConverter;

public class ConvertFixedEmployeesIgnoreAdditionalFields {
	
	public static void main (String [] args) throws IOException, FlatReaderException {
		FixedLengthToXmlConverter parser = 
			new FixedLengthToXmlConverter (
					new FileOutputStream (new File ("files/employees_adflds.xml")));
		parser.setFirstLineAsHeader (true);
		parser.getStrategy().setIgnoreEmptyLines (false);
		parser.getStrategy().setCommentStart ('#');
		parser.getStrategy().setLengths (new int [] {5, 7, 2});
		parser.getStrategy().setIgnoreAdditionalFields(true);
		parser.read (new InputStreamReader(new FileInputStream (new File ("files/employees_adflds.csv"))));
	}
	
}
